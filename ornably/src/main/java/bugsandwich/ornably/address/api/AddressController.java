...
>>>>>>> 369775c85ab8eae3c91ad0437c15fc3fd5a92adb
...
//		)).collect(Collectors.toList());
	...
	public ResponseEntity<?> patchMyAddress(@PathVariable Integer addressPk, // url에 있는 주소PK
		...
		addressDTO.setAddressPk(addressPk);
		addressDTO.setCondition("UPDATE_DEFAULT_ADDRESS_BY_ACCOUNT_PK_AND_ADDRESS_PK");
		...
		// 결과에 따라 응답
		if (addressService.updateAddress(addressDTO)) {
			// 내 주소가 아니거나,존재하지 않거나, 수정 불가능한경우
			// 에러상태 출력
			return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
		}
...
//============배송지등록 ==============
	...
	public ResponseEntity<?> registAddress(@RequestBody AddressDTO addressDTO, // 등록할 주소 정보
		...
		// 로그인사용자PK
		addressDTO.setAccountPk(ornablyUser.getAccountPk());
		addressDTO.setCondition("INSERT_ADDRESS");
