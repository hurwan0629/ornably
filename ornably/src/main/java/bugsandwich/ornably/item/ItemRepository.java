...
public class ItemRepository {
	...
	private static final String DECREASE_ITEM_STOCK_BY_CART =
	        "UPDATE ITEM I SET I.ITEM_STOCK = I.ITEM_STOCK - ( " +
	        "    SELECT C.CART_COUNT FROM CART C " +
	        "    WHERE C.ITEM_PK = I.ITEM_PK AND C.ACCOUNT_PK = ? " + // 해당 회원 장바구니 수량 조회
	        ") " +
	        "WHERE EXISTS ( " +
	        "    SELECT 1 FROM CART C " +
	        "    WHERE C.ITEM_PK = I.ITEM_PK AND C.ACCOUNT_PK = ? " + // 장바구니에 해당 상품이 있을 때만 실행
	        ") " +
	        "AND I.ITEM_STOCK >= ( " +
	        "    SELECT C.CART_COUNT FROM CART C " +
	        "    WHERE C.ITEM_PK = I.ITEM_PK AND C.ACCOUNT_PK = ? " + // 재고가 장바구니 수량 이상일 때만 차감
	        ")";
	...
	public boolean update(ItemDTO itemDTO) {
	    ...
	    // 장바구니에 담긴 수량만큼 재고 차감
	        ...
	        result = jdbcTemplate.update(DECREASE_ITEM_STOCK_BY_CART, itemDTO.getAccountPk(), itemDTO.getAccountPk(), itemDTO.getAccountPk());
	    }
